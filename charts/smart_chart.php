...
$sql = mysqli_query($conn, "SELECT COUNT(final_status) AS paid_status FROM smart_table WHERE final_status = 'PAID'");
while ($row = $sql->fetch_assoc()) {
    $count = $row['paid_status'];
}
$sql = mysqli_query($conn, "SELECT COUNT(final_status) AS transmitted_status FROM smart_table WHERE final_status = 'TRANSMITTED'");
while ($row = $sql->fetch_assoc()) {
    $count1 = $row['transmitted_status'];
}
$sql = mysqli_query($conn, "SELECT COUNT(final_status) AS unpaid_status FROM smart_table WHERE final_status = 'UNPAID'");
while ($row = $sql->fetch_assoc()) {
    $count2 = $row['unpaid_status'];
}

$sql = mysqli_query($conn, "SELECT COUNT(*) AS smart_status FROM smart_table");
while ($row = $sql->fetch_assoc()) {
...
<body>
    <div class="row">
        <div class="col-5 text-center">
            <div class="chart">
                <canvas id="chart-smart" class="chart-canvas" height="197"></canvas>
            </div>
            <h4 class="font-weight-bold mt-n8">
                <span id="smart_stats" countto="<?php echo $smart_status; ?>"><?php echo $smart_status; ?></span>
    ...
    <script>
        var ctx1 = document.getElementById("chart-smart").getContext("2d");
