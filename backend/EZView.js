...
$.fn.EZView = function (collectionName) {
  ...
  self.constructor = function () {
    ...
    $thumbnails.each(function (i, thumbnail) {
      ...
      try {
        ...
        self.arContent[collectionName][i].href =
          $thumbnail.attr("href") || $thumbnail.attr("src");
        self.arContent[collectionName][i].name = $thumbnail.attr("title")
          ? $thumbnail.attr("title").substring(0, 30)
          : "";
  ...
  self.returnImageToOriginalMeasures = function (index) {
    ...
    var $img = $(
        "[index-content=" + collectionName + index + "]",
        self.$EZView
      ),
      hasOriginalSizes =
        self.arContent[collectionName][index]["original-height"];

    if (hasOriginalSizes) {
      $img.animate(
        {
          height:
            self.arContent[collectionName][index]["original-height"] + "px",
  ...
  self.createContainer = function () {
    if (!$("#EZView" + collectionName).length) {
      var tools =
          ...
          '<img src="' +
          self.icons.zoomIn +
          '" title="Zoom in [hotkey +]" /></a>' +
          '<a class="zoomOut EZViewHighlight" href="">' +
          '<img src="' +
          self.icons.zoomOut +
          '" title="Zoom out [hotkey -]" /></a>' +
          "</spam>" +
        ...
        container =
          ...
          '<a class="back EZViewHighlight"><img src="' +
          self.icons.back +
          '" title="Back [hotkey <]" /></a>' +
          '<a class="next EZViewHighlight"><img src="' +
          self.icons.next +
          '" title="Next [hotkey >]" /></a>' +
          tools,
        template =
  ...
  self.buildHtmlContent = function () {
    ...
    if (isPdf) {
      htmlContent =
        '<iframe class="content" frameborder="0" index-content="' +
        collectionName +
        imgIndex +
        '" height="' +
        $(window).height() * 0.95 +
        ...
        '" src="' +
        src +
        '" type="application/pdf"><p>Your browser does not support iframes.</p>' +
  ...
  self.setContentOnViewer = function (htmlContent) {
    ...
    $("[index-content=" + collectionName + newIndex + "]").on(
      "error",
      function () {
        var style =
          "padding: 10px; border-radius: 10px; background-color: rgba(255,255,255,0.6)";

        $(this).replaceWith(
          '<h1 class = "content" index-content="' +
            collectionName +
            self.index[collectionName] +
            '" style="' +
            style +
            '">Unsupported preview</h1>'
        );
  ...
  self.zoom = function (increment) {
    ...
    var $img = $(
        "[index-content=" + collectionName + self.index[collectionName] + "]",
        self.$EZView
      ),
    ...
    if (
      !self.arContent[collectionName][self.index[collectionName]][
        "original-height"
      ]
    ) {
      self.arContent[collectionName][self.index[collectionName]][
  ...
  self.next = function () {
    ...
    // Check if the element exists
    var hrefExist = $(
        '[href="' + self.arContent[collectionName][newIndex].href + '"]:visible'
      ).length,
      srcExist = $(
        '[src="' + self.arContent[collectionName][newIndex].href + '"]:visible'
      ).length,
